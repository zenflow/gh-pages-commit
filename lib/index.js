...
var ghPagesCommit = function(dir, options, cb){
	...
	getGitIgnore(function(error){
		...
		getGitStatus(function(error, status){
			...
			if (previous_branch!='gh-pages'){
				exec('git checkout --orphan gh-pages', next);
			} else {
				next(null);
			}
			function next(error){
				if (error){return cb(error);}
				// remove all files from working copy
				removeFiles(function(error){
					if (error){return cb(error);}
					// save .gitignore
					saveGitIgnore(function(error){
						if (error){return cb(error);}
						// copy files from dir to basedir
						copyFiles(function(error){
							if (error){return cb(error);}
							// commit all files
							commitFiles(function(error){
								if (error){return cb(error);}
	...
	function removeFiles(cb){
		exec('git rm -rf "'+path.relative(process.cwd(), basedir)+'"', cb);
	}
	function saveGitIgnore(cb){
		fs.writeFile(path.join(basedir, '.gitignore'), gitignore, cb);
	}
	function copyFiles(cb){
		exec('cp -r "'+path.join(basedir, dir)+'/**/**" "'+basedir+'/**/**"', cb);
	}
	function commitFiles(cb){
		exec('git add '+path.relative(process.cwd(), basedir)+' && git commit -m "automatic commit"', cb);
	}
};
