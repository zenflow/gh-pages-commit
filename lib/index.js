		...
		function copyFiles(cb){
			fs.readdir(path.join(basedir, dir), function (error, files) {
				...
				async_each(files, function(file, cb){
					exec('cp -r "'+path.join(basedir, dir, file)+'" "'+path.join(basedir, file)+'"', cb);
				}, cb);
			});
		},
		function commitFiles(cb){
			exec('git add .'+path.sep+path.relative(process.cwd(), basedir)+' && git commit -m "automatic commit (zenflow rules)"', cb);
		},
