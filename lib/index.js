	...
	getGitIgnore(function(error){
		...
		getGitStatus(function(error, status){
			...
			function next(error){
				...
				exec('git rm -rf "'+path.relative(process.cwd(), basedir)+'"', function(error){
					if (error){return cb(error);}
					// save .gitignore
					saveGitIgnore(function(error){
						...
						exec('cp -r "'+path.join(basedir, dir)+'/" "'+basedir+'/"', function(error){
							if (error){return cb(error);}
							// commit all files
							exec('git add '+path.relative(process.cwd(), basedir)+' && git commit -m "automatic commit"', function(error){
								if (error){return cb(error);}
